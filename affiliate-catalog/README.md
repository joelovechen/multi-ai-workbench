...
5. Commit `catalog.json`, `catalog.sig`, and any authorized icons together on `main`; Cloudflare Pages follows `main/docs` automatically.
6. Refresh the GitHub Pages mirror by rebuilding the `gh-pages` branch from `docs` (for example, `git subtree split --prefix docs` and then a fast-forward/force-with-lease update after inspecting the target).
7. Confirm both Pages endpoints return the same catalog bytes and signature before announcing the change.
