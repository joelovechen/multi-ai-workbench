...
- 只有在用户主动选择对话导出方式后，扩展才读取受支持 AI 网页中的当前会话。为避免虚拟列表造成消息缺失，扩展优先在当前登录页面内调用该平台自身的历史接口或 RPC，并按平台规则处理分页和当前分支；专用链路不可用时才向上滚动并进行 DOM 降级提取。页面请求所需的令牌、Cookie 和请求头只在当前页面内存中临时使用，不写入扩展存储、不记录日志，也不发送给开发者服务器。会话内容通过 `chrome.storage.session` 临时传给扩展预览页，预览关闭或浏览器会话结束后清理。
- 用户选择 Base64 或资源包图片策略时，扩展会直接从对话中已有的图片或附件地址读取资源并在本地生成文件；这些请求仍由对应第三方站点处理，不经过开发者服务器。
