...
try {
  ...
  if (await consentContinue.isVisible()) {
    const browserUiLanguage = await page.evaluate(() => chrome.i18n.getUILanguage());
    const expectedLocale = browserUiLanguage.toLowerCase().startsWith("zh") ? "zh" : "en";
    const initialLocale = await page.locator("#maiwLocale").inputValue();
    if (initialLocale !== expectedLocale) throw new Error(`first_use_locale_not_browser_default:${browserUiLanguage}:${initialLocale}`);
    await page.selectOption("#maiwLocale", "zh"); await page.selectOption("#maiwPetScope", "supported"); await consentContinue.click();
  }
  ...
  if (await page.locator("[data-action=hide]").count()) throw new Error("affiliate_hide_action_must_only_exist_in_settings");
  ...
  const launcherHitArea = await platformPage.evaluate(() => { const host = document.querySelector("#multi-ai-floating-launcher-host"), number = (key) => Number(host.dataset[key]), x = number("positionX"), y = number("positionY"), hitLeft = number("hitLeft"), hitTop = number("hitTop"), hitWidth = number("hitWidth"), hitHeight = number("hitHeight"), renderWidth = number("renderWidth"), renderHeight = number("renderHeight"), outside = document.elementFromPoint(x + Math.max(2, hitLeft / 2), y + hitTop + hitHeight / 2), inside = document.elementFromPoint(x + hitLeft + hitWidth / 2, y + hitTop + hitHeight / 2), rightGap = innerWidth - (x + hitLeft + hitWidth); return { hitLeft, hitTop, hitWidth, hitHeight, renderWidth, renderHeight, rightGap, edgeGap: number("edgeGap"), edgeSnap: host.dataset.edgeSnap, outsideIntercepted: outside === host, insideIntercepted: inside === host }; });
  ...
  if (launcherHitArea.rightGap !== 4 || launcherHitArea.edgeGap !== 4 || launcherHitArea.edgeSnap !== "true") throw new Error(`floating_launcher_default_edge_invalid:${JSON.stringify(launcherHitArea)}`);
  await page.evaluate(async () => { const stored = await chrome.storage.local.get("maiw.settings"), settings = stored["maiw.settings"] || {}; await chrome.storage.local.set({ "maiw.settings": { ...settings, launcherEdgeGap: 18, launcherEdgeSnap: false } }); });
  await platformPage.waitForFunction(() => { const host = document.querySelector("#multi-ai-floating-launcher-host"), x = Number(host?.dataset.positionX), hitLeft = Number(host?.dataset.hitLeft), hitWidth = Number(host?.dataset.hitWidth); return host?.dataset.edgeGap === "18" && host.dataset.edgeSnap === "false" && innerWidth - (x + hitLeft + hitWidth) === 18; });
  ...
  await reopenedWorkspace.evaluate(async () => { const stored = await chrome.storage.local.get(["maiw.settings", "maiw.operations"]), settings = stored["maiw.settings"] || {}, operations = (stored["maiw.operations"] || []).filter((row) => row.name !== "冒烟测试操作"); await chrome.storage.local.set({ "maiw.settings": { ...settings, locale: "en" }, "maiw.operations": operations }); });
  await reopenedWorkspace.reload({ waitUntil: "domcontentloaded" }); await reopenedWorkspace.waitForFunction(() => document.documentElement.lang === "en" && document.querySelector("#send")?.textContent === "Send to all");
  const englishWorkspaceHan = await reopenedWorkspace.evaluate(() => { const allowed = /豆包|腾讯元宝|智谱清言|千问|知乎直答|百度|微信|知乎|搜狗|旧会话|旧问题|冒烟测试操作/; const values = []; const walker = document.createTreeWalker(document.body, NodeFilter.SHOW_TEXT); for (let node = walker.nextNode(); node; node = walker.nextNode()) { const value = node.nodeValue.trim(); if (/[\u3400-\u9fff]/.test(value) && !allowed.test(value) && !node.parentElement?.closest("script,style")) values.push(value); } for (const node of document.querySelectorAll("[title],[aria-label],[placeholder]")) for (const name of ["title", "aria-label", "placeholder"]) { const value = node.getAttribute(name) || ""; if (/[\u3400-\u9fff]/.test(value) && !allowed.test(value)) values.push(`${name}:${value}`); } return [...new Set(values)]; });
  const englishSidePage = await context.newPage(); await englishSidePage.goto(`chrome-extension://${extensionId}/sidepanel/index.html`, { waitUntil: "domcontentloaded" }); await englishSidePage.waitForFunction(() => document.documentElement.lang === "en");
  const englishSideHan = await englishSidePage.evaluate(() => { const allowed = /豆包|腾讯元宝|智谱清言|千问|知乎直答/; const values = []; const walker = document.createTreeWalker(document.body, NodeFilter.SHOW_TEXT); for (let node = walker.nextNode(); node; node = walker.nextNode()) { const value = node.nodeValue.trim(); if (/[\u3400-\u9fff]/.test(value) && !allowed.test(value) && !node.parentElement?.closest("script,style")) values.push(value); } return [...new Set(values)]; });
  if (englishWorkspaceHan.length || englishSideHan.length) throw new Error(`english_ui_contains_untranslated_text:${JSON.stringify({ workspace: englishWorkspaceHan, sidepanel: englishSideHan })}`);
  await reopenedWorkspace.click("#settingsToggle"); await reopenedWorkspace.selectOption("#locale", "zh"); await reopenedWorkspace.waitForFunction(() => document.documentElement.lang === "zh-CN" && document.querySelector("#send")?.textContent === "同时提问" && document.querySelector('[data-settings-tab="general"]')?.textContent === "常规");
  const chineseRoundTrip = await reopenedWorkspace.evaluate(() => ({ petSetting: [...document.querySelectorAll("strong")].some((node) => node.textContent === "网页桌宠显示范围"), builtinAction: document.querySelector("#promptTemplateList")?.textContent.includes("中英双向翻译") }));
  if (!chineseRoundTrip.petSetting || !chineseRoundTrip.builtinAction) throw new Error(`chinese_ui_round_trip_failed:${JSON.stringify(chineseRoundTrip)}`);
  console.log(JSON.stringify({ extensionId, screenshotPath, sideScreenshotPath, affiliateScreenshotPath, affiliateCatalogVerified: true, defaultServices, defaultSideService, defaultSideLoadedFrames, sidepanelServices: sidepanelServiceKeys, sidepanelManager, adaptiveServices, layout: "2x3", platformPanels: frameTops.length, visualRows: new Set(frameTops).size, serviceCards, composerMoved: true, petAnimationAssets: petAssetMetadata.length, launcherInjected, launcherAnimated, launcherHitArea, launcherImageMode, compactSideLayout, modelTabDesign, adaptiveLayout, sidePanelVisibleFrames: visibleSideFrames, sidePanelParallelSend: true, promptTemplateApplied: true, builtInPresetCount, customTemplateSaved: true, sidePanelLayout, workspaceClosedOnSideSwitch: true, workspaceOpenedOnFullSwitch: true, safeWorkspaceUrlRestore: true, englishUiAudit: true, chineseRoundTrip: true }));
} finally {
