(function initWorkbench() {
  ...
  const state = { services: [...registry.defaults], serviceDraft: [], serviceCategory: "ai", layout: "auto", theme: "light", locale: "", openBehavior: "resume", answerMode: "expert", launcherEnabled: true, launcherScope: "supported", launcherStyle: "animated", launcherSize: 160, launcherEdgeGap: 4, launcherEdgeSnap: true, launcherAnimationPack: "rich", launcherRandomFrequency: "normal", launcherSingleAction: "sidepanel", launcherDoubleAction: "workspace", launcherTripleAction: "export-center", contextMenuMode: "flat", conversationExport: { showContextMenu: true, popupPosition: "pet", includeThinking: false, showTimestamp: false, includeSource: true, defaultFormat: "markdown", imageMode: "link", filenameRule: "{title}-{platform}-{date}", theme: "auto", fontFamily: "system", fontSize: 16, contentWidth: 820, pageSize: "A4", orientation: "portrait", margin: 16, scale: 1, pageNumbers: false }, operations: [], operationGroups: [], commandShortcuts: {}, composerPosition: null, widths: {}, frameUrls: {}, history: [], currentSessionId: "", files: [], lastQuestion: "", selections: [], selectionService: "", pickerServices: new Set(), workspaceTabId: null, workspaceWindowId: null };
  ...
  function showSettingsTab(tab) { const selected = ["general", "operations", "export"].includes(tab) ? tab : "general"; $("#settingsGeneral").hidden = selected !== "general"; $("#operationSettings").hidden = selected !== "operations"; $("#conversationExportSettings").hidden = selected !== "export"; for (const button of $("#settingsTabs").querySelectorAll("button")) button.classList.toggle("active", button.dataset.settingsTab === selected); if (selected === "operations") renderPromptTemplates(); }
  ...
  async function initialize() {
    ...
    if (settings.conversationExport && typeof settings.conversationExport === "object") state.conversationExport = { ...state.conversationExport, ...settings.conversationExport };
    ...
    const openSettings = (await chrome.storage.session.get("maiw.openSettings"))["maiw.openSettings"]; if (["templates", "operations", "general", "export"].includes(openSettings)) { await chrome.storage.session.remove("maiw.openSettings"); openDrawer(elements.settingsPanel); showSettingsTab(openSettings === "templates" ? "operations" : openSettings); }
  ...
  const exportSettingBindings = { exportContextMenu: ["showContextMenu", "checked"], exportPopupPosition: ["popupPosition", "value"], exportIncludeThinking: ["includeThinking", "checked"], exportShowTimestamp: ["showTimestamp", "checked"], exportIncludeSource: ["includeSource", "checked"], exportDefaultFormat: ["defaultFormat", "value"], exportImageMode: ["imageMode", "value"], exportFilenameRule: ["filenameRule", "value"], exportTheme: ["theme", "value"], exportFontFamily: ["fontFamily", "value"], exportFontSize: ["fontSize", "number"], exportContentWidth: ["contentWidth", "number"], exportPageSize: ["pageSize", "value"], exportOrientation: ["orientation", "value"], exportMargin: ["margin", "number"], exportScale: ["scale", "number"], exportPageNumbers: ["pageNumbers", "checked"] };
  for (const [id, [key, type]] of Object.entries(exportSettingBindings)) $("#" + id).addEventListener("change", async (event) => { state.conversationExport[key] = type === "checked" ? event.target.checked : type === "number" ? Number(event.target.value) : event.target.value; await persistSettings(); setStatus("对话导出设置已保存。"); });
